...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    component = EntityComponent[InputList](_LOGGER, DOMAIN, hass)
    ...
    collection.sync_entity_lifecycle(
        hass, DOMAIN, DOMAIN, component, yaml_collection, InputList
    )
    ...
    collection.sync_entity_lifecycle(
        hass, DOMAIN, DOMAIN, component, storage_collection, InputList
    )
    ...
    component.async_register_entity_service(
        SERVICE_SET_VALUE, {vol.Required(CONF_VALUE): cv.string}, "async_set_value"
    )
    component.async_register_entity_service(
        SERVICE_INCREMENT, {vol.Required(CONF_KEY): cv.string, vol.Required(CONF_VALUE): cv.string}, "async_increment"
    )
    component.async_register_entity_service(
        SERVICE_DECREMENT, {vol.Required(ATTR_VALUE): cv.string}, "async_decrement"
    )
...
class InputList(collection.CollectionEntity, RestoreEntity):
    ...
    async def async_increment(self, listkey, listvalue):
        """Increment."""
        if len(listvalue) < self._minimum or len(listvalue) > self._maximum:
            _LOGGER.warning(
                "Invalid value: %s (length range %s - %s)",
                listvalue,
                self._minimum,
            ...
            return
        self._attrs[listkey] = listvalue
        self._current_value = self._current_value + 1
        self.async_write_ha_state()

    async def async_decrement(self, listkey):
        """Decrement."""
        if len(listkey) < self._minimum or len(listkey) > self._maximum:
            _LOGGER.warning(
                "Invalid value: %s (length range %s - %s)",
                listkey,
                self._minimum,
            ...
            return
        try:
            self._attrs.pop(listkey)
            self._current_value = self._current_value - 1
            self.async_write_ha_state()
        except KeyError:
            _LOGGER.error ("Ivalid key: %s", listkey)
